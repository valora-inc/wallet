...
export async function callCloudFunction(functionName: string, data: {} = {}) {
  ...
  const url = `https://us-central1-${process.env.GCLOUD_PROJECT}.cloudfunctions.net/${functionName}`
  // Uncomment to test locally
  // const url = `http://localhost:5000/${process.env.GCLOUD_PROJECT}/us-central1/${functionName}`
