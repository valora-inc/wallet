...
3. Increase the schema version in [src/redux/store.ts](packages/mobile/src/redux/store.ts#L27)
4. Add a new migration in [src/redux/migrations.ts](packages/mobile/src/redux/migrations.ts)
5. Add a new test schema in [test/schema.ts](packages/mobile/test/schema.ts), with the newly added/deleted/updated properties. The test schema is useful to test migrations and show how the schema changed over time.
6. Optional: if the migration is not trivial, add a test for it in [src/redux/migrations.test.ts](packages/mobile/src/redux/migrations.test.ts)
...
[rootstate]: packages/mobile/src/redux/reducers.ts#L79
[rootstateschema]: packages/mobile/test/RootStateSchema.json
