...
export class ValoraWalletConnectClient  {
  private client: WalletConnectClient

  constructor () {
    this.client = WalletConnectClient.init({ relayProvider: ''})
   ...
   onSessionPayload = async (event: SessionTypes.PayloadEvent) => {
    const {
      topic,
  ...
  return {
    init: async (uri: string) => {
      ...
      client.on(CLIENT_EVENTS.session.proposal, put(SessionCreated()))
