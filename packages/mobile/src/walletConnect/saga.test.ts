  ...
  describe('initialiseWalletConnect', () => {
    ...
    it('initializes v1 if enabled', async () => {
      await expectSaga(initialiseWalletConnect, v1ConnectionString, origin)
        .provide([
          [select(walletConnectEnabledSelector), { v1: true, v2: false }],
          [call(initialiseWalletConnectV1, v2ConnectionString, origin), {}],
        ])
    ...
    it('doesnt initialize v1 if disabled', async () => {
      await expectSaga(initialiseWalletConnect, v1ConnectionString, origin)
        .provide([
          [select(walletConnectEnabledSelector), { v1: false, v2: true }],
          [call(initialiseWalletConnectV1, v2ConnectionString, origin), {}],
        ])
