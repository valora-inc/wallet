...
describe('Home Feed :ios:', () => {
  it('should load more items on scroll', async () => {
    ...
    const startingItems = await element(by.id('TransactionListItem')).getAttributes()
  ...
  it('should show correct information on tap of feed item', async () => {
    ...
    const items = await element(
      by.id('TransferFeedItem').withAncestor(by.id('TransactionListItem'))
    ).getAttributes()

    // Tap top TransferFeedItem
    await element(by.id('TransferFeedItem').withAncestor(by.id('TransactionListItem')))
      .atIndex(0)
      .tap()
