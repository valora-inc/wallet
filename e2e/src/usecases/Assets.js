...
async function validateSendFlow(tokenSymbol) {
  ...
  await waitForElementById({ testID: 'SendSelectRecipientSearchInput', tap: true })
  ...
  await waitForElementById({ testID: 'SendOrInviteButton', tap: true })
  await expect(
...
export default Assets = () => {
  describe.each([
    ...
    it('navigates to wallet tab from home', async () => {
      await waitForElementById({ testID: 'Tab/Wallet', tap: true })
      await waitForElementById({ testID: 'Assets/TabBar' })
    })
    ...
    it('switching tabs displays corresponding assets', async () => {
      ...
      await waitForElementById({ testID: 'Assets/NoNfts' })
    ...
    describe.each(tokens)('For $symbol', ({ symbol, tokenId, learnMore, actions, moreActions }) => {
      it('navigates to asset details on tapping asset', async () => {
        await waitForElementById({ testID: `TokenBalanceItemTouchable/${tokenId}`, tap: true })
        await waitForElementById({ testID: 'TokenDetails/AssetValue' })
      })
      ...
      if (moreActions.includes('Send')) {
        it('send action under more actions navigates to send flow', async () => {
          ...
          await waitForElementById({ testID: 'TokenDetailsMoreActions/Send', tap: true })
          await validateSendFlow(symbol)
          await waitForElementById({ testID: 'TokenDetails/AssetValue' })
        })
      }
      ...
      if (moreActions.includes('Add')) {
        it('add action under more actions navigates to add cico flow', async () => {
          await element(by.id('TokenDetails/Action/More')).tap()
          await waitForElementById({ testID: 'TokenDetailsMoreActions/Add', tap: true })
          await validateAddFlow(symbol)
          await waitForElementById({ testID: 'TokenDetails/AssetValue' })
        })
      }
      ...
      if (moreActions.includes('Withdraw')) {
        it('withdraw action under more actions navigates to withdraw spend screen', async () => {
          await element(by.id('TokenDetails/Action/More')).tap()
          await waitForElementById({ testID: 'TokenDetailsMoreActions/Withdraw', tap: true })
          await waitForElementById({ testID: 'FiatExchangeTokenBalance' })
          await element(by.id('BackChevron')).tap()
          await waitForElementById({ testID: 'TokenDetails/AssetValue' })
        })
      }
      ...
      if (learnMore) {
        it('learn more navigates to coingecko page', async () => {
          ...
          await waitForElementById({ testID: 'TokenDetails/LearnMore', tap: true })
          ...
          await waitForElementById({ testID: 'TokenBalanceItem' })
        })
      }
