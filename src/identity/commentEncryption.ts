...
import { TokenTransaction, TokenTransactionTypeV2, isTokenTransfer } from 'src/transactions/types'
...
export function* checkTxsForIdentityMetadata({ transactions }: TransactionFeedUpdatedAction) {
  try {
...
function findIdentityMetadataInComments(
  ...
  for (const tx of transactions) {
    if (!isTokenTransfer(tx) || tx.type !== TokenTransactionTypeV2.Received) {
      continue
    }
