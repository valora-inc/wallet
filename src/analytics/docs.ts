...
export const eventDocs: Record<AnalyticsEventType, string> = {
  ...
  [AppEvents.account_funded]: `When a user's balance has changed from below 1 cent (USD) to above. May not fire if user leaves app before funds land and restarts before re-opening. User must enter app for this to get published (like all events in this file).`,
  [AppEvents.account_liquidated]: `When a user's balance changes from above 1 cent (USD) to below. May not fire if user leaves app before funds land and restarts before re-opening. User must enter app for this to get published (like all events in this file).`,
