...
import { SendRedesignSearchInput } from 'src/send/SendRedesignSearchInput'
...
function GetStartedSection() {
  ...
  const options = [
    {
      optionNum: '1',
      title: t('sendRedesign.getStarted.options.one.title'),
      subtitle: t('sendRedesign.getStarted.options.one.subtitle'),
    },
    {
      optionNum: '2',
      title: t('sendRedesign.getStarted.options.two.title'),
      subtitle: t('sendRedesign.getStarted.options.two.subtitle'),
    },
    {
      optionNum: '3',
      title: t('sendRedesign.getStarted.options.three.title'),
      subtitle: t('sendRedesign.getStarted.options.three.subtitle'),
    },
    {
      optionNum: '4',
      title: t('sendRedesign.getStarted.options.four.title'),
      subtitle: t('sendRedesign.getStarted.options.four.subtitle'),
    },
  ]

  return (
    <View style={getStartedStyles.container} testID={'SelectRecipient/GetStarted'}>
      <Text style={getStartedStyles.subtitle}>{t('sendRedesign.getStarted.subtitle')}</Text>
      <Text style={getStartedStyles.title}>{t('sendRedesign.getStarted.title')}</Text>
...
function SendRedesign() {
  ...
  const shouldShowGetStarted = !recentRecipients.length
  ...
  useAsync(async () => {
    if (shouldImportContacts) {
      // TODO: Currently if a user does not have a phone number linked, this silently fails
  ...
  const onPressContacts = () => {
    ValoraAnalytics.track(SendEvents.send_redesign_invite)
    setShouldImportContacts(true)
  }

  const onPressQR = () => {
    ValoraAnalytics.track(SendEvents.send_redesign_scan_qr)
  ...
  const buildSections = (): Section[] => {
    const sections = [{ key: t('sendRedesign.recents'), data: recentRecipients }].filter(
  ...
  return (
    <SafeAreaView style={styles.body} edges={['top']}>
      <View style={styles.header}>
        ...
        />
        <SendRedesignSearchInput input={searchQuery} onChangeText={throttledSearch} />
      </View>
      <View style={styles.content}>
        <View style={styles.topSection}>
          <Text style={styles.title}>{t('sendRedesign.title')}</Text>
          <SelectRecipientButton
            testID={'SelectRecipient/QR'}
            title={t('sendRedesign.qr.title')}
            subtitle={t('sendRedesign.qr.subtitle')}
            onPress={onPressQR}
            icon=<QRCode />
          />
          <SelectRecipientButton
            testID={'SelectRecipient/Invite'}
            title={t('sendRedesign.invite.title')}
            subtitle={t('sendRedesign.invite.subtitle')}
...
SendRedesign.navigationOptions = {
  ...noHeader,
  ...Platform.select({
