...
info_plist_strings="ios/mobileStackRuntime/Base.lproj/InfoPlist.strings"
echo -e "/* DO NOT EDIT MANUALLY, SEE scripts/sync_ios_info_plist_strings.sh */\n" > $info_plist_strings

for plist_key in "${plist_keys[@]}"; do
  plist_value=$(/usr/libexec/PlistBuddy -c "Print :$plist_key" ios/mobileStackRuntime/Info.plist)
